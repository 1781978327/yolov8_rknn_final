...
int STrack::max_trail_length = 100;  // 默认100帧（约4秒@25fps）
...
void STrack::re_activate(STrack &new_track, int frame_id, bool new_id)
{
	...
	trajectory.push_back({tlwh[0] + tlwh[2] * 0.5f, tlwh[1] + tlwh[3] * 0.5f});
	if (trajectory.size() > max_trail_length) trajectory.pop_front();
...
int STrack::next_id()
{
	...
	return _count.fetch_add(1, std::memory_order_relaxed) + 1;
}
