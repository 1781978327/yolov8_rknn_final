...
Eigen::MatrixXf NearNeighborDisMetric::_pdist(const FEATURESS & x, const FEATURESS & y)
{
    ...
    if (len1 == 0 || len2 == 0) return Eigen::MatrixXf::Zero(len1, len2);
    ...
    const FEATURESS & a, const FEATURESS & b, bool data_is_normalized)
{
    ...
    if (!data_is_normalized) {
        for (int i = 0; i < a.rows(); ++i) aa.row(i) = a.row(i) / sqrt(a.row(i).squaredNorm());
        for (int i = 0; i < b.rows(); ++i) bb.row(i) = b.row(i) / sqrt(b.row(i).squaredNorm());
    }
    return 1. - (aa * bb.transpose()).array();
}
